{ stage0env
, nyacc-tar-gz ? builtins.fetchurl { url = "https://download.savannah.gnu.org/releases/nyacc/nyacc-1.00.2.tar.gz";
...
}: stage0env.override (final: prev: with final;
{
  ...
  src = builtins.fetchurl
        { url = "https://mirrors.kernel.org/gnu/${pname}/${name}.tar.gz";
          sha256="7ddae0854e46ebfa18c13ab37e64839a7b86ea88aeed366a8d017efd11dae86e";
        };
  outputs = [ "out" "dev" ];
  ...
  buildCommand = ''
    ...
    mkdir -p ''${bindir} ''${libdir} ''${incdir} ''${datadir}

    MES_PREFIX=''${NIX_BUILD_TOP}/build/${MES_PKG_DIR}
    GUILE_LOAD_PATH=''${MES_PREFIX}/mes/module:''${MES_PREFIX}/module:''${NIX_BUILD_TOP}/build/${NYACC_PKG}/module
    ...
    mkdir src build
    cd src
    ungz --file ${nyacc-tar-gz} --output ${NYACC_PKG}.tar
    ungz --file ${src} --output ${MES_PKG}.tar

    # Save a copy of sources in datadir.
    cd ''${datadir}
    untar --file ''${NIX_BUILD_TOP}/src/${NYACC_PKG}.tar
    untar --non-strict --file ''${NIX_BUILD_TOP}/src/${MES_PKG}.tar # ignore symlinks

    # This copy will be used for an in-place build.
    ...
    cp ${./files/config.h} ''${MES_PREFIX}/include/mes/config.h
    ...
    rm ''${datadir}/${MES_PKG_DIR}/mes/module/mes/psyntax.pp ''${datadir}/${MES_PKG_DIR}/mes/module/mes/psyntax.pp.header
    ...
    cp ''${datadir}/${MES_PKG_DIR}/mes/module/srfi/srfi-9-struct.mes ''${datadir}/${MES_PKG_DIR}/mes/module/srfi/srfi-9.mes
    cp ''${datadir}/${MES_PKG_DIR}/mes/module/srfi/srfi-9/gnu-struct.mes ''${datadir}/${MES_PKG_DIR}/mes/module/srfi/srfi-9/gnu.mes

    # Move some modules into standard location
    cp ''${datadir}/${MES_PKG_DIR}/module/mes/getopt-long.scm ''${datadir}/${MES_PKG_DIR}/mes/module/mes/
    cp ''${datadir}/${MES_PKG_DIR}/module/mes/guile.scm ''${datadir}/${MES_PKG_DIR}/mes/module/mes/
    cp ''${datadir}/${MES_PKG_DIR}/module/mes/optargs.scm ''${datadir}/${MES_PKG_DIR}/mes/module/mes/

    # Fixes to support newer M2-Planet
    catm x86_defs.M1 ${../stage0/M2libc/x86/x86_defs.M1} lib/m2/x86/x86_defs.M1
    ...
    sha256sum \
        ...
        ''${bindir}/mes-m2 \
        ''${bindir}/mescc.scm \
        ''${prefix}/lib/x86-mes/crt1.s \
        ''${prefix}/lib/x86-mes/crt1.o \
        ''${prefix}/lib/x86-mes/x86.M1 \
        ''${prefix}/lib/x86-mes/libmescc.s \
        ''${prefix}/lib/x86-mes/libc+tcc.s \
        ''${prefix}/lib/x86-mes/libc.s \
        ''${prefix}/lib/x86-mes/libmescc.a \
        ''${prefix}/lib/x86-mes/libc+tcc.a \
        ''${prefix}/lib/x86-mes/libc.a \
        ''${prefix}/lib/linux/x86-mes/elf32-header.hex2 \
        ''${prefix}/lib/linux/x86-mes/elf32-footer-single-main.hex2
  '';
